...
class PlayerJump : EventListener<JumpEvent>() {
    ...
    override suspend fun complete(event: JumpEvent) {
        if (event.creature.properties.values.getBoolean(IS_CLIMBING) && event.creature.climbThing != null) {
            event.creature.display { "${event.creature.asSubject(it)} jump from ${event.creature.climbThing?.name} in ${event.source}" }
        } else {
