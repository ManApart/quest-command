...
//TODO - take into account blocking
class Attack : EventListener<AttackEvent>() {
    ...
    override fun execute(event: AttackEvent) {
        ...
        if (weaponRange < targetDistance) {
            ...
            processAttack(defender, event, damageSource, offensiveDamage)
    ...
    private fun processAttack(defender: Creature, event: AttackEvent, damageSource: String, offensiveDamage: Int) {
        ...
        if (attackedPart == null) {
            ...
            processAttackHit(event, adjustedPosition, attackedPart, subject, defenderName, defenderPosition, damageSource, defender, offensiveDamage)
        }
    }

    private fun processAttackHit(event: AttackEvent, adjustedPosition: TargetPosition, attackedPart: BodyPart, subject: String, defenderName: String, defenderPosition: TargetPosition, damageSource: String, defender: Creature, offensiveDamage: Int) {
