...
private val eventListeners: List<core.events.EventListener<*>> = listOf(crafting.DiscoverRecipe(), status.statChanged.StatMinned(), quests.journal.ViewQuestList(), inventory.dropItem.ItemDropped(), status.conditions.RemoveCondition(), use.actions.ScratchSurface(), combat.approach.StartMove(), traveling.arrive.ArrivalHandler(), combat.battle.CombatantDied(), combat.takeDamage.TakeDamage(), combat.block.Block(), system.persistance.loading.ListSaves(), system.debug.DebugTagListener(), use.actions.StartFire(), inventory.ListInventory(), core.properties.propValChanged.PropertyStatMinned(), use.actions.NoUseFound(), quests.CompleteQuest(), system.message.DisplayMessage(), use.actions.DamageCreature(), magic.ViewWordHelp(), status.ExpGained(), quests.journal.ViewQuestJournal(), traveling.RestrictLocation(), magic.castSpell.StartCastSpell(), traveling.jump.PlayerJump(), use.eat.EatFood(), status.status.Status(), use.actions.ChopWood(), traveling.travel.TravelStart(), combat.dodge.StartDodge(), status.rest.Rest(), traveling.routes.FindRoute(), combat.block.StartBlock(), combat.attack.Attack(), crafting.checkRecipe.CheckRecipes(), status.statChanged.CreatureDied(), status.effects.ApplyEffects(), traveling.scope.remove.RemoveItem(), use.interaction.nothing.DoNothing(), combat.approach.Move(), system.history.ViewChatHistory(), status.statChanged.StatChanged(), inventory.equipItem.EquipItem(), quests.QuestListener(), inventory.pickupItem.ItemPickedUp(), inventory.unEquipItem.ItemUnEquipped(), use.actions.UseItemOnIngredientRecipe(), inventory.dropItem.TransferItem(), system.debug.DebugToggleListener(), system.debug.DebugListListener(), combat.dodge.Dodge(), magic.castSpell.CastSpell(), crafting.craft.Craft(), time.gameTick.GameTick(), system.help.ViewHelp(), inventory.unEquipItem.UnEquipItem(), explore.Look(), traveling.routes.ViewRoute(), traveling.scope.spawn.ActivatorSpawner(), core.target.item.ItemSpawner(), use.interaction.nothing.StartNothing(), traveling.scope.remove.RemoveScope(), traveling.arrive.Arrive(), use.actions.UseIngredientOnActivatorRecipe(), traveling.climb.ClimbComplete(), use.actions.UseOnFire(), combat.battle.BattleTick(), combat.attack.StartAttack(), status.statChanged.PlayerStatMinned(), time.gameTick.TimeManager(), traveling.scope.spawn.SpawnItem(), status.conditions.AddCondition(), system.persistance.loading.Load(), core.GameManager.MessageHandler(), explore.map.ReadMap(), status.statChanged.StatBoosted(), use.interaction.Interact(), core.TurnListener(), status.LevelUp(), inventory.equipItem.ItemEquipped(), traveling.jump.PlayerFall(), status.statChanged.PlayerStatMaxed(), traveling.move.Move(), core.properties.propValChanged.PropertyStatChanged(), core.history.SessionListener(), use.actions.UseFoodItem(), combat.battle.BattleTurn(), system.debug.DebugStatListener(), core.properties.SetProperties(), quests.SetQuestStage(), traveling.climb.AttemptClimb(), use.interaction.NoInteractionFound(), system.persistance.saving.Save())
